...
export default function CheckInsPage() {
    ...
    const [semiFinalistIds, setSemiFinalistIds] = useState<Set<string>>(new Set());
    ...
    useEffect(() => {
        // Fetch check-ins and semi-finalist IDs in parallel
        Promise.all([
            fetchCheckIns(),
            fetchSemiFinalistIds(),
    ...
    const fetchSemiFinalistIds = async () => {
        try {
            const token = getAdminToken();
            const res = await fetch(
                `${process.env.NEXT_PUBLIC_API_URL}/admin/semi-finalists`,
                { headers: { Authorization: `Bearer ${token}` } }
            );
            const data = await res.json();
            const ids = new Set<string>((data.semi_finalists || []).map((sf: { team_id: string }) => sf.team_id));
            setSemiFinalistIds(ids);
        } catch (err) {
            console.error('Failed to fetch semi-finalists:', err);
        }
    };

    const filteredTeams = teams.filter(team => {
        if (semiFinalistIds.has(team.id)) return false; // exclude semi-finalists
    ...
    const isAllSelected = filteredTeams.length > 0 && filteredTeams.every(t => selectedIds.has(t.id));
    const isPartialSelected = filteredTeams.some(t => selectedIds.has(t.id)) && !isAllSelected;
